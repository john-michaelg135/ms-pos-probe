...
export function Sidebar() {
  ...
  // Filter nav items by the current user's role (owners see everything).
  const role = session?.role;
  const allowedNavItems = mainNavItems.filter((item) =>
    canAccessRoute(role as never, item.href)
  );

  // POS-PROBE has no dedicated /settings page yet — keep the footer link hidden.
                ...
                <DropdownMenuTrigger asChild>
                  <div className="flex items-center justify-between p-2 rounded-lg hover:bg-sidebar-accent cursor-pointer transition-colors group">
                    <div className="flex flex-col min-w-0 pr-2">
                      ...
                      <button className="text-xs font-semibold bg-brand-500 text-white px-2 py-1 rounded truncate mt-0.5">
                        Analytics Dashboard
                      </button>
                    </div>
                ...
                >
                  ...
                  {systems.map((sys) => {
                    ...
                    return (
                      <DropdownMenuItem
                        key={sys.fullName}
                        className="cursor-pointer text-xs flex items-center justify-between py-2.5 px-2 hover:bg-accent font-medium"
                      >
