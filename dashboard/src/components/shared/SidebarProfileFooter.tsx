...
import { User, Shield, Activity, LogOut, MoreVertical } from "lucide-react";
...
export function SidebarProfileFooter() {
  ...
  if (!session?.user) {
    return (
      <div className="flex items-center gap-2.5 p-2 rounded-xl text-xs text-muted-foreground animate-pulse">
        Loading session...
      </div>
    );
  }
    ...
    <div className="w-full">
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <div className="flex items-center gap-2.5 p-2 rounded-xl hover:bg-sidebar-accent transition-colors cursor-pointer group">
            ...
            <div className="flex-1 overflow-hidden space-y-0.5">
              ...
              <p className="text-xs font-medium text-muted-foreground truncate leading-tight capitalize">
                {role}
              </p>
            </div>
        ...
        <DropdownMenuContent
          className="w-64 bg-popover border-border text-popover-foreground z-[99999] shadow-xl p-2"
          side="top"
          align="start"
        >
          <div className="p-2 bg-muted/40 rounded-lg border border-border mb-2 flex items-center gap-2">
            ...
            <div className="flex flex-col min-w-0 overflow-hidden">
              ...
              <Badge
                variant="outline"
                className="text-[9px] w-fit mt-1 px-1.5 py-0 font-semibold capitalize"
              >
                {role}
              </Badge>
            </div>
