...
import { Printer, FileText, TrendingUp, ShieldAlert, Calendar, MapPin, Filter } from "lucide-react";
...
export default function ReportsPage() {
  ...
  const forecast = useQuery({
    queryKey: ["report", "forecast", 7],
    queryFn: () => fetchForecast(7),
    enabled: category === "all" || category === "forecast",
  });

  const forecastInsights = useQuery({
    queryKey: ["report", "forecast-insights", 7],
    queryFn: () => fetchForecastInsights(7),
    ...
    <div className="space-y-6 page-enter">
      ...
      <div className="hidden print:block print:mb-6">
        ...
        <p className="text-sm text-center text-gray-400 mt-1">
          Generated on {reportDate} • Period: {formatDate(dateFrom)} – {formatDate(dateTo)}
        </p>
      ...
      {(category === "all" || category === "forecast") && (
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 animate-slide-up stagger-2">
            ...
            <MetricCard label="Forecast Window" value="Next 7 Days" sublabel="Predicted demand period" />
          </div>
          ...
          {forecast.data && forecast.data.length > 0 && (
            <div className="bg-white dark:bg-[#1a2231] border border-gray-200 dark:border-[#2d3748] rounded-2xl p-6 animate-bounce-in stagger-3">
              <h3 className="text-[14px] font-semibold text-gray-900 dark:text-gray-50 mb-2">Demand Forecast (Next 7 Days)</h3>
